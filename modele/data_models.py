...
class models_indeed():
    ...
    def __init__(self):
        ...
        self.models_param = {'REG_LOG' :{'penalty' : ['l1', 'l2']},
                            ...
                            'GBOOST': {'n_estimators':  [1,5,10], 'learning_rate': [0.01, 0.1, 1],
                                       'max_depth': [1, 2]},
                                       
                            'SVM_RBF': {'kernel': ['rbf'],'C': [0.01,0.1,1, 10], 
                            ...
                            'ADABOOST': {'n_estimators':  [10, 50], 'learning_rate': [0.1, 1, 10]}
}
...
masque_dev = (df['métier_sc'] != 'developpeur')
masque_devops = (df['métier_sc'] != 'devops')
df = df[masque_dev]
df = df[masque_devops]
df['métier_sc'].value_counts()
df.head()
df.shape
...
titre_matrice_corr_salaire.head(20)
...
# # <center>  Modèles Titre + métier + contrat + localisation </center> 
...
# # <center> Modele Description </center>
...
# #   <center>  Modeles Titre + Description  </center>
...
# # Voting Classifier (features = Titre +Métier +Contrat +Localisation +Gs )
...
df_ensemble = pd.DataFrame({'Voting_Classifier':[accuracy_train,accuracy_test,fscore]},index=['Accuracy_train','Accuracy_test','Fscore'])
df_ensemble['Features']  = 'Titre +Métier +Contrat +Localisation +Gs'                                                                                          
df_ensemble=df_ensemble.round(2)


# # Prediction des valeurs manquantes salaires
...
pd.get_dummies(df['métier_sc']).shape
pd.get_dummies(df['loc_sc']).shape
pd.get_dummies(df['contrat']).shape
...
df_final = df_final.drop(['apprentissage','intérim'],axis=1)
...
csv_data['range_salaire'] = csv_data['Salaire'].apply(range_salaire)
csv_data['x_min'] = csv_data['range_salaire'].apply(x_min)
csv_data['x_max'] = csv_data['range_salaire'].apply(x_max)

csv_data.head()

if not os.path.exists('data'):
    os.mkdir('data')
csv_data.to_csv('data\csv_data.csv',index=False)